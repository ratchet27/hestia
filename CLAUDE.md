...
- **NEVER** use a subset (e.g. bare `mago format && mago lint`) as the backend gate → run **`make lint`**. It runs `rector → mago format → mago lint → mago analyze → phpstan` — the same five checks CI's Code Quality job runs (CI runs each in check mode; `make lint` auto-fixes locally). Any subset skips a gate: bare `mago lint` skips `mago analyze` (analyzer errors like `invalid-return-statement`, `mixed-argument`, …) and also the rector and phpstan gates — all pass locally but fail CI.
