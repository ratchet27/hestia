...
**Fix (Option A):**
- Add an application timezone resolved by a small **timezone resolver** service (resolved **once at construction and cached**, not per-request):
  - **Config default** `app.timezone` (env `APP_TIMEZONE`) = **`+05:00`** — the always-correct fixed-offset fallback.
  - **Preferred zone** = `Asia/Almaty` (named). At startup the resolver checks what `Asia/Almaty` resolves to in this runtime for a date after 2024-03-01:
    - if it resolves to **+05:00** → use the **named zone** `Asia/Almaty` (future-proof; auto-tracks any genuine IANA change);
    - otherwise (stale tzdata → +06, or any other offset) → fall back to the **fixed `+05:00`** from config.
  - **Why:** Kazakhstan moved permanently to UTC+5 on 2024-03-01 and observes **no DST**, so `+05:00` is semantically exact. The named zone resolves to +05 *only* with tzdata ≥ 2024a; a FrankenPHP container with **stale tzdata silently resolves Almaty to +06** and schedules every chore a day off. This resolver gets the named zone's future-proofness when tzdata is trustworthy and the fixed offset's safety when it isn't — automatically.
  - **Guard test:** assert the resolver's effective timezone yields a **+05:00** offset for a post-2024-03-01 date, so a misconfigured/stale-tzdata environment **fails loudly** instead of scheduling wrong.
